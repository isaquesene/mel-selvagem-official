...
const Header = () => {
  ...
  return (
    <header className={`${isActive ? 'bg-primary-400 py-[16px]' : 'bg-transparent py-[20px]'} fixed z-30 left-0 right-0 mx-auto flex justify-between items-center px-[20px] lg:px-[80px] transition-all duration-300`}>
      {/* logo */}
      <a href='/#'>
        <img className='lg:h-[60px] h-[45px] m-[5px] scale-[1.3] ' src={logo} alt=''/>
      ...
      <div className='hidden lg:flex space-x-4'>
        <a href={link} className={'btn btn-sm cursor-pointer shadow-md' + (isActive ? 'btn-secondary' : 'btn-primary')} target='_blank'>{btnLoginText}</a>
